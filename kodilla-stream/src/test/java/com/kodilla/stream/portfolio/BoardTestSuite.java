...
public class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask(){
        /*
        Celem zadania jest obliczenie średniej liczby dni, jaka upłynęła od zlecenia wykonania zadania do
        dnia bieżącego w liście zadań, które są aktualnie w trakcie realizacji. Do jego wykonania można użyć dwóch
        strumieni – jeden aby obliczyć sumę, a drugi, aby obliczyć liczbę elementów. Lub w wersji trudniejszej –
        należy wykorzystać kolektor skalarny average()
         */
        ...
        double result = project.getTaskLists().stream()
                ...
                .flatMap(x->x.getTasks().stream()).map(x->x.getCreated()).mapToInt(x->x.until(LocalDate.now()).getDays())
                .average().getAsDouble();
